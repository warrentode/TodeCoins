...
public class TypeAwareSellItemForItemsFactory implements SerializableTradeOfferFactory {
    ...
    public TypeAwareSellItemForItemsFactory(Map<VillagerType, ItemStack> buyMap1, Optional<Map<VillagerType, ItemStack>> buyMap2, Map<VillagerType, ItemStack> sellMap, int maxUses, int experience, float multiplier) {
        BuiltInRegistries.VILLAGER_TYPE.stream()
                .filter(villagerType ->
                        !buyMap1.containsKey(villagerType) ||
                                (buyMap2.isPresent() && !buyMap2.get().containsKey(villagerType)) ||
                                !sellMap.containsKey(villagerType)
                )
                .findAny()
                .ifPresent(villagerType -> {
                    ResourceLocation id = BuiltInRegistries.VILLAGER_TYPE.getKey(villagerType);
                    throw new IllegalStateException("Missing trade for villager type: " + id);
                });
